...
export default {
	...
	data: function () {
	    return {
	        ...
	        maxCardHeight: this.cards.map(() => {return {height: 'auto'};}),
	        cardsDesktopPosition: this.cards.map(() => {return {css: {top: '0px', left: '0px'}, rawTop: 0};}),
	...
	methods: {
		...
		updateLayoutOnResize: function () {
		    this.maxCardHeight = this.cards.map(() => {return {height: 'auto'};});
            nextTick(() => {
                if(this.$refs.card) {
                    if(this.isWidthCompact) {
                        const maxCardHeight = Math.max(...this.$refs.card.map((cardItem) => domHeight(cardItem.$el)));
                        this.maxCardHeight = this.cards.map(() => {return {height: (maxCardHeight-(settings.cardBorder*2))+'px'};});
                    } else {
                        const columnHeights = range(settings.desktopCardsPerRow).map(() => 0);
                        const temporaryCardsDesktopPosition = this.cards.map(() => {return {css: {top: '0px', left: '0px'}, rawTop: 0};});
